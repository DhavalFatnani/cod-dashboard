-- Update KPI metrics function to include test orders
-- This allows simulator/test data to be visible in the dashboard
...
BEGIN
  ...
  -- MECE Principle: Only count active COD orders (COD_HARD and COD_QR), excluding CANCELLED and RTO
  SELECT 
  ...
  SELECT jsonb_build_object(
    ...
    'cod', (
      ...
      FROM public.orders
      WHERE payment_type = 'COD'
        AND cod_type IN ('COD_HARD', 'COD_QR')  -- MECE: Only count active COD orders
        AND (p_start_date IS NULL OR created_at >= p_start_date)
        AND (p_end_date IS NULL OR created_at <= p_end_date)
        AND (p_store_id IS NULL OR store_id = p_store_id)
        AND (p_rider_id IS NULL OR rider_id = p_rider_id)
